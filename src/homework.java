...
public class homework {
	public static void main(String[] args) throws FileNotFoundException{
		...
		int numOfActiveLines = Integer.parseInt(in.next());
		...
		for(int i = 0;i < numOfIntersection; i++){
			graph.addVertex(in.next(),Integer.parseInt(in.next()));
		}
		in.close();
	...
	private static void searchWithBfs_New(Graph graph) throws FileNotFoundException{
		...
		while(!frontier.isEmpty()){
			...
			for(Node child: getChildren(current,graph)){
				...
				else if(!child.stateCheck(frontier)){//this is checked but not possible 
	...
	//Implemented a merge sort for Frontier queue in both UCS and A* search.
	//Recursive top down.
	...
	public static Queue<Node> mergeAStar(Queue<Node> left,Queue<Node> right,Queue<Node> frontier){
		//A* is sorted based on hueristic
		Queue<Node> result = new LinkedList<Node>();
		ArrayList<Node> reference = toArrayList(frontier);
		while(!left.isEmpty() && !right.isEmpty()){
	...
	//Node and Graph class implementation
	...
	//could be used in Space optimization
	...
	//Main Node for Search
	public static class Node{
		private String intersection;
		private int pathCost;
		private Node parent;
		private int depth;				//depth is the value for sort in DFS BFS, although non explicit.
		private int sunPathCost;
		private int gValue;				//for Uniform cost search
		private int hueristic;			//for A* search
		private LinkedList<Node> children = new LinkedList<Node>();
		
		//Node constructor general
		...
		//Constructor with explicit depth and gValue assignment
		...
		//compare by A* search critier: gValue + Sunday PathCost which is the hueristic here.
